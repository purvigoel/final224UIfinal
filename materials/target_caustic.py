...
mask = color.rgb2gray(io.imread("/Users/purvigoel/224Final/images/op.png"))
...
eps = 1
firsttime = 0
while(i < 2000 and attempt < 200):
	r = random.randint(1, 10)
	attempt += 1
	vector = []
	if(r < 5 or firsttime == 0 ):
		vectorLength = random.randint(math.floor(image.shape[0]/2), 1500) #50
	...
	#diff = np.sum(np.abs(caustic - desired)) + len(vector)
	if attempt > 50:
	...
	if(printcounter == 500):
		printcounter = 0
		printImage = bestImage.copy()
		printImage[bestImage < 0]= 0
		printImage[bestImage>1] = 1
		io.imsave("/Users/purvigoel/224Final/images/bestMap.jpg", img_as_ubyte(printImage))
		io.imsave("/Users/purvigoel/224Final/images/bestCaustic.jpg", img_as_ubyte(bestCaustic))

bestImage[bestImage < 0]= 0
bestImage[bestImage>1] = 1
